        ...
        <div class="col-12 col-md-12 col-lg-12">
          <div class="card">
            <div class="card-header">
              <a href="<?= base_url() ?>OrderProyek_c/addOP" class="btn btn-icon icon-left btn-primary note-btn" id="kegiatan_t">
            ...
            <div class="card-body">
              <div class="table-responsive">
                <table class="table table-striped" id="op_tabel" style="width:100%">
                  <thead>
                    <tr>
                      <th>
                        #
...
<script>
  ...
  var tabel = $("#op_tabel").DataTable({
    ...
    serverSide: true,
    ajax: {
      "url": "<?php echo base_url() . 'OrderProyek_c/ignited_data' ?>",
      "type": "POST"
    },
    columns: [{
        "data": null,
  ...
  $(document).on('click', '#op_d', function() {
    console.log('delete');
    isSave = 0;
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        $.ajax({
          type: "POST",
          url: "<?= base_url() ?>OrderProyek_c/hapus/" + id,
        ...
        notifgagal('Data Order Proyek', 'dihapus');
      }
    })
  });
  ...
  $(document).on('click', '#op_detail', function() {
    ...
    $.ajax({
      type: "GET",
      dataType: "JSON",
      url: "<?= base_url(); ?>OrderProyek_c/form/" + id,
      success: function(respon) {
        $(".modal-body").html(respon.view);
        $('.modal-title').text('Detail Order Proyek');
        $('#modal').modal('show');
      }
    });
  });
