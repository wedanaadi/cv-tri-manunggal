  ...
  <tbody>
    <?php foreach ($lap['jn'] as $v) : ?>
      <tr>
        ...
        <td style="border-left: 1px solid; border-right: 1px solid;"><?= number_format($v->harga, 0, ',', '.') ?></td>
        <td style="border-left: 1px solid; border-right: 1px solid;"><?= number_format($v->jumlah, 0, ',', '.') ?></td>
      </tr>
    <?php endforeach; ?>
    <tr>
      <td colspan=" 4" style="border: 1px solid"><strong>JUMLAH</strong></td>
      <td style="width: 150px; border: 1px solid"><?= number_format($lap['atas']->jumlah_harga, 2, ',', '.') ?></td>
    </tr>
    <tr>
      <td style="border: 1px solid" colspan=" 4"><strong>PPN 10%</strong></td>
      <td style="border: 1px solid"><?= number_format($lap['atas']->ppn, 2, ',', '.') ?></td>
    </tr>
    <tr>
      <td style="border: 1px solid" colspan=" 4"><strong>Jumlah Total</strong></td>
      <td style="border: 1px solid"><?= number_format($lap['atas']->total_harga_satuan, 2, ',', '.') ?></td>
    </tr>
    <tr>
      <td style="border: 1px solid" colspan=" 4"><strong>Harga Per- Bulah (PEMBULATAN)</strong></td>
      <td style="border: 1px solid"><?= number_format(bulatkan($lap['atas']->total_harga_satuan), 0, ',', '.') ?></td>
    </tr>
    <tr>
      <td style="border: 1px solid" colspan=" 4"><strong>Volume ( Unit )</strong></td>
      <td style="border: 1px solid"><?= $lap['atas']->volume . " (" . $lap['atas']->satuan . ")" ?></td>
    </tr>
    <tr>
      <td style="border: 1px solid" colspan=" 4"><strong>Total Harga</strong></td>
      <td style="border: 1px solid"><?= number_format($lap['atas']->total, 2, ',', '.') ?></td>
    </tr>
    <tr>
      <td style="border: 1px solid" colspan=" 4"><strong>Total Harga (PEMBULATAN)</strong></td>
      <td style="border: 1px solid"><?= number_format(bulatkan($lap['atas']->total), 0, ',', '.') ?></td>
    </tr>
  </tbody>
</table>
