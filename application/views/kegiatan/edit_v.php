...
<script>
  $(function() {
    ...
    $.each(editedit, function(k, v) {
      ...
      tabel.row.add([
        v.tipe,
        v.tipe !== 'option3' ? v.nama_barang_upah : v.upah_bahan_alat,
        v.satuan,
        v.koef,
        v.harga,
        upah,
    ...
    $(document).on('click', '#ubah', function(e) {
      ...
      if (tb[0] === 'option1') {
        ...
        select2isi('[name=barang]', tb[9], tb[1]);
        $("[name=upah]").val('').trigger('change');
        $("[name=alat]").val('');
        $('[name=upah],[name=alat]').prop('disabled', true);
        ...
        $('[name=koeu]').val(tb[3]);
        $('[name=hsu]').val(tb[4]);
        select2isi('[name=upah]', tb[9], tb[1]);
        $("[name=barang]").val('').trigger('change');
        $("[name=alat]").val('');
        $('[name=barang],[name=alat]').prop('disabled', true);
      } else {
    ...
    $(document).on('click', '#submit', function(e) {
      ...
      if (form[0].checkValidity() === false || detailtabel === null) {
        ...
        $.ajax({
          ...
          url: "<?= base_url() ?>Kegiatan_c/update/" + dataedit.id_master_kegiatan,
          success: function(respon) {
            notifsukses('Data Kegiatan', 'diubah');
            setTimeout(function() {
